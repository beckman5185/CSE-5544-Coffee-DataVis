...
with ui.navset_tab():
    ...
    # Third tab for Spend Traits
    with ui.nav_panel("Spend Traits"):
        with ui.layout_sidebar():
            with ui.sidebar():
                ui.input_selectize(
            ...
            with ui.card():
                #proportion heatmap
                ui.card_header("Price graph")
                @render_plotly
                def priceMap():
                    import plotly.express as px

                    #get non-null preference data
                    preferenceData = coffeeSurvey[coffeeSurvey['total_spend'].notnull()]

                    #group by level of demographic variable
                    spendGroups = preferenceData.groupby(input.spend())['total_spend'].value_counts(normalize=True)
                    spendGroups = spendGroups.reset_index()

                    spendGroups = spendGroups.sort_values(by=[input.spend(), 'total_spend'])
                    
                    #make grouped bar chart
                    return px.bar(spendGroups, x=input.spend(), y='proportion', color='total_spend', barmode='group', color_discrete_sequence=px.colors.qualitative.Prism)                    # Third tab for Spend Traits
    with ui.nav_panel("Spend Traits"):
        with ui.layout_sidebar():
            with ui.sidebar():
                ui.input_selectize(
            ...
            with ui.card():
                ...
                def priceMap():
                    ...
                    #make grouped bar chart
                    return px.bar(spendGroups, x=input.spend(), y='proportion', color='total_spend', barmode='group', color_discrete_sequence=px.colors.qualitative.Prism)